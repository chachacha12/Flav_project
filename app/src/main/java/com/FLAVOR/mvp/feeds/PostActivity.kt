...
class PostActivity : BasicActivity() {
    ...
    fun init(){
        ...
        val comments = intent.getStringExtra("comments") //해당 게시물의 댓글들 목록 스트링으로 가져옴
        Log.e("태그", "포스트액티비티로 받아온 유저, 태그값들: "+user+ tag1+tag2+tag3)

        val user_json = JSONObject(user!!)  //받아온 string값을 jsonobject로 변경
        contents!!.User =user_json      //contents의 User요소를 덮어씌움. 가져온걸로.
        val tag1_json = JSONObject(tag1!!)
        contents!!.Tag_FirstAdj =tag1_json
        val tag2_json = JSONObject(tag2!!)
        contents!!.Tag_SecondAdj =tag2_json
        val tag3_json = JSONObject(tag3!!)
        contents!!.Tag_Location =tag3_json

        val comments_json = JSONArray(comments)
        ...
        server.comment_upload_Request(
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    ...
                    new_comment.put("kakao_id",Usersingleton.kakao_id)  //이걸 넣어줘야 업로드하자마자 댓글삭제할때 삭제로직가능
