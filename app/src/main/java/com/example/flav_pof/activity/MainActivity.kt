...
class MainActivity : BasicActivity() {
    ...
    fun init() {
        ...
        try {
            ...
            Log.e("태그", "Initialized Amplify")
        } catch (error: AmplifyException) {
            Log.e("태그그", "Could not initialize mplify", error)
        }

        //현재 유저의 세션정보가 있는지 확인해줌
    ...
    fun storeUploader(id: String) {
        if (successCount == 0) {  //스토리지가 제대로 다 삭제되었을때만 db안의 내용도 삭제할거임.
            ...
            firebaseFirestore.collection("posts")
                ...
                .addOnFailureListener {
                    Toast.makeText(this@MainActivity, "게시글을 삭제하지 못하였습니다.", Toast.LENGTH_SHORT).show()
                }
        }
